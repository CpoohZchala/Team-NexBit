...
const RatingsWidget = () => {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        const avgResponse = await axios.get('http://localhost:3000/api/ratings/average');
        setAverageRating(avgResponse.data.averageRating);
      } catch (error) {
        console.error('Error fetching ratings:', error);
      }
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      {/* Optional: Add a message if you want to indicate recent ratings are hidden */}
      {/* <p className="text-gray-500 text-sm">Recent ratings are currently not displayed.</p> */}
    </div>
  );
};
